...
Beyond the [transformed effects](/effects/transformable), SineVFX can drive plain Roblox effect
instances directly. Select them and the [Emit window](/windows/emit) can **emit**, **enable**,
and **repeat** them, and they all ship through the [runtime](/shipping/api) the same way.

## What SineVFX can emit

| Object                              | How it fires                                              |
| ----------------------------------- | --------------------------------------------------------- |
| **3D particle** (transformed Part)  | Spawns part-particles. See [3D particle](/effects/part).  |
...
| **Camera** (transformed)            | Drives the [camera effect](/effects/camera).              |
...
Every emittable object responds to the same controls in the [Emit window](/windows/emit):
...
- **Repeat** - keep firing on a timer (see [Emit & preview](/windows/emit#repeat)).

## Firing nested effects on spawn

Inside a [3D particle's](/effects/part) template you can nest plain effect instances and have
...
All of these are driven by the [runtime module](/shipping/module) through the same
`VFX.emit / enable / disable` calls. See the [Runtime API](/shipping/api).
