...
- adds a **`Properties` folder** of grouped settings that the
  [Properties window](/properties) edits.

[Untransforming](/transform) removes the tag and template and gives you the plain
instance back.
...
The [Emit window](/emit) drives these in edit mode; the
[runtime API](/api) exposes the exact same three in your game.
...
[runtime module](/module) - and plants it in `ReplicatedStorage.SineVFX`. You call
...
emit / enable / disable controls. See [Emittable objects](/emittable).
