...
straight into the [graph editor](/windows/graph-editor).
...
- **Colours** - open the [Color tools](/tools/color) for a full picker, palettes, and
...
- **Number sequences** - a value that varies over lifetime. Type it as `top,bottom` to set a
  value with an envelope, or open the **[graph editor](/windows/graph-editor)** for full
  curve control.
...
[graph editor](/windows/graph-editor). That's where transparency fades, size curves, and
...
- Pair this window with the [Emit window](/windows/emit) open so you can see each change
